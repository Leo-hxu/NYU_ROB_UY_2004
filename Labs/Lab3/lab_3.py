...
class InverseKinematics(Node):
    ...
    def get_error_leg(self, theta, desired_position):
        theta = np.array(theta, dtype=float)  # Fix
        desired_position = np.array(desired_position, dtype=float)  # Fix
        current_position = self.leg_forward_kinematics(theta)  # Fix
        error_vec = current_position - desired_position  # Fix
        return float(error_vec @ error_vec)  # Fix
    ...
    def inverse_kinematics_single_leg(self, target_ee, leg_index, initial_guess=[0, 0, 0]):
        ...
        target_ee = np.array(target_ee, dtype=float)  # Fix
        x0 = np.array(initial_guess, dtype=float)  # Fix

        result = scipy.optimize.minimize(  # Fix
