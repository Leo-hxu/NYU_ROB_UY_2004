...
class InverseKinematics():
    ...
    def __init__(self):
        ...
        ################################################################################################
        touch_down_position = np.array([0.05, 0.0, -0.14])  # Fix
        stand_position_1 = np.array([0.025, 0.0, -0.14])  # Fix
        stand_position_2 = np.array([0.0, 0.0, -0.14])  # Fix
        stand_position_3 = np.array([-0.025, 0.0, -0.14])  # Fix
        liftoff_position = np.array([-0.05, 0.0, -0.14])  # Fix
        mid_swing_position = np.array([0.0, 0.0, -0.05])  # Fix
        ...
        rf_ee_triangle_positions = np.array([
            touch_down_position,  # Fix
            stand_position_1,  # Fix
            stand_position_2,  # Fix
            stand_position_3,  # Fix
            liftoff_position,  # Fix
            mid_swing_position,  # Fix
        ]) + rf_ee_offset
        ...
        self.target_joint_positions_cache, self.target_ee_cache = self.cache_target_joint_positions()  # Fix
        print(f'shape of target_joint_positions_cache: {self.target_joint_positions_cache.shape}')  # Fix
        print(f'shape of target_ee_cache: {self.target_ee_cache.shape}')  # Fix
    ...
    def get_error_leg(self, theta, desired_position):
        ...
        desired_position = np.array(desired_position, dtype=float)

        current_position = self.leg_forward_kinematics(theta)
        error_vec = current_position - desired_position
        return float(error_vec @ error_vec)
    ...
    def inverse_kinematics_single_leg(self, target_ee, leg_index, initial_guess=[0, 0, 0]):
        ...
        target_ee = np.array(target_ee, dtype=float)
    ...
    def interpolate_triangle(self, t, leg_index):
        waypoints = self.ee_triangle_positions[leg_index]  # Fix
        num_points = len(waypoints)  # Fix
        t_wrapped = t % 1.0  # Fix
        segment_float = t_wrapped * num_points  # Fix
        segment_index = int(np.floor(segment_float)) % num_points  # Fix
        segment_t = segment_float - np.floor(segment_float)  # Fix

        p0 = waypoints[segment_index]  # Fix
        p1 = waypoints[(segment_index + 1) % num_points]  # Fix
        return (1.0 - segment_t) * p0 + segment_t * p1  # Fix
...
def main():
    ...
    if len(theta_list) > 0:  # Fix
        theta_arr = np.array(theta_list)  # Fix
        plt.plot(theta_arr[:, 0])  # Fix
        plt.plot(theta_arr[:, 1])  # Fix
        plt.plot(theta_arr[:, 2])  # Fix
        plt.xlabel('Step')  # Fix
        plt.ylabel('Joint Angle (rad)')  # Fix
        plt.legend(['theta_1', 'theta_2', 'theta_3'])  # Fix
        plt.title('IK Joint Angles for Front Right Leg')  # Fix
        plt.show()  # Fix
    ...
    if len(inverse_kinematics.target_ee_cache):
        ...
        for position in inverse_kinematics.target_ee_cache:
            x_list.append(position[3])  # Fix
            z_list.append(position[5])  # Fix
        plt.xlabel('X(m)')
        plt.ylabel('Z(m)')
        plt.title('EE front left foot trot gait')  # Fix
